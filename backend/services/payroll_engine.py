...
class PayrollCalculationEngine:
    ...
    async def fetch_rule_based_deductions(
        ...
        """
        Fetch deductions from the UNIFIED employee_penalties collection.
        Only penalties with status='approved' are included in payroll.
        
        Single source: employee_penalties collection WHERE status = 'approved'
        """
        employee_id = employee.get("id")
        deductions = []
        
        # Single query: all approved penalties for this employee/month
        # Includes both direct penalties AND arrears carried forward
        penalties = await self.db.employee_penalties.find({
            "$or": [
                {"employee_id": employee_id, "month": month, "status": "approved", "is_arrears": {"$ne": True}},
                {"employee_id": employee_id, "effective_month": month, "status": "approved", "is_arrears": True}
            ]
        }, {"_id": 0}).to_list(50)
        ...
        for penalty in penalties:
            ...
            source = penalty.get("source", "manual")
            
            calc = self._log_calculation(
                component_name=f"{penalty.get('name', 'Penalty')}{arrear_label}",
                input_values={
                    "rule_id": penalty.get("rule_id") or penalty.get("violation_code"),
                    "reason": penalty.get("reason"),
                    "violation_code": penalty.get("violation_code"),
                    ...
                    "approved_by": penalty.get("approved_by_name", "HR"),
                    "approved_at": penalty.get("approved_at")
                },
                formula=f"{'Arrears: ' if is_arrear else ''}{source} penalty (approved)",
                output_value=penalty.get("amount", 0),
                rule_id=penalty.get("violation_code") or penalty.get("rule_id", "PENALTY"),
                rule_version="1.0"
            )
            deductions.append({
                "key": f"penalty_{penalty.get('violation_code', 'unknown')}{'_arrears' if is_arrear else ''}_{penalty.get('id', '')[:8]}",

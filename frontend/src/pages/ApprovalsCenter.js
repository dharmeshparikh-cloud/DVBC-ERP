  ...
  return (
    <div data-testid="approvals-center" className={isDark ? 'text-zinc-100' : ''}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-4 mb-6">
        ...
        {isHR && (
          <Card className={`${isDark ? 'border-zinc-700 bg-zinc-800' : 'border-zinc-200'} shadow-none rounded-lg`}>
            <CardContent className="p-2.5 md:p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0">
                  <p className={`text-[10px] md:text-xs uppercase truncate ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Profile</p>
                  <p className="text-lg md:text-2xl font-semibold text-blue-600">{profileChangeApprovals.length}</p>
                </div>
                <User className="w-5 h-5 md:w-8 md:h-8 text-blue-500/30 flex-shrink-0" />
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {/* Expense Approvals stat */}
        {(isManager || isHR) && (
          <Card className={`${isDark ? 'border-zinc-700 bg-zinc-800' : 'border-zinc-200'} shadow-none rounded-lg`}>
            <CardContent className="p-2.5 md:p-4">
              <div className="flex items-center justify-between">
                <div className="min-w-0">
                  <p className={`text-[10px] md:text-xs uppercase truncate ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Expense</p>
                  <p className="text-lg md:text-2xl font-semibold text-emerald-600">{expenseApprovals.length}</p>
                </div>
                <Receipt className="w-5 h-5 md:w-8 md:h-8 text-emerald-500/30 flex-shrink-0" />
              </div>
            </CardContent>
          </Card>
        )}
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <div className={`text-xs mt-1 font-medium ${isDark ? 'text-orange-400' : 'text-orange-600'}`}>
                        <span className="flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                          Pending approval from: <strong>Admin</strong> (Go-Live Employee)
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700"
                        disabled={actionLoading}
                        data-testid={`approve-kickoff-${request.id}`}
                      >
                        <CheckCircle className="w-4 h-4 mr-1" /> Approve
                      </Button>
                      <Button
                        ...
                        disabled={actionLoading}
                        data-testid={`reject-kickoff-${request.id}`}
                      >
                        <XCircle className="w-4 h-4 mr-1" /> Reject
                      </Button>
                    </div>
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                    <div className="flex-1">
                      ...
                      <div className={`text-xs mt-1 font-medium ${bank.status === 'pending_hr' ? (isDark ? 'text-amber-400' : 'text-amber-600') : (isDark ? 'text-blue-400' : 'text-blue-600')}`}>
                        <span className="flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                          Pending approval from: <strong>{bank.status === 'pending_hr' ? 'HR Manager' : 'Admin'}</strong>
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
        ...
        <DialogContent className={`${isDark ? 'border-zinc-700 bg-zinc-900' : 'border-zinc-200'} rounded-lg max-w-3xl max-h-[90vh] overflow-y-auto`}>
          ...
          {selectedGoLive && (
            <div className="space-y-4">
              ...
              {/* Pre-Flight Checklist - Dynamically render from API */}
              <div>
                <h4 className={`text-sm font-semibold mb-3 ${isDark ? 'text-zinc-300' : 'text-zinc-700'}`}>
                  Go-Live Checklist
                </h4>
                    ...
                    <>
                      {Object.entries(goLiveChecklist.checklist).map(([key, item]) => (
                        <div key={key} className={`flex items-center justify-between p-3 rounded-lg border ${
                          ...
                          <div className="flex items-center gap-3">
                            {key === 'personal_details' && <User className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-red-500'}`} />}
                            {key === 'official_email' && <User className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-red-500'}`} />}
                            {key === 'department_assigned' && <Building2 className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-red-500'}`} />}
                            {key === 'reporting_manager' && <User className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-red-500'}`} />}
                            {key === 'bank_details' && <CreditCard className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-red-500'}`} />}
                            {key === 'bank_verified' && <Shield className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-amber-500'}`} />}
                            {key === 'documents_uploaded' && <FileText className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-red-500'}`} />}
                            {key === 'portal_access' && <Key className={`w-5 h-5 ${item.completed ? 'text-emerald-500' : 'text-red-500'}`} />}
                            <span className={isDark ? 'text-zinc-100' : 'text-zinc-900'}>{item.label}</span>
                          </div>
                          {item.completed 
              ...
              {/* Summary based on actual checklist */}
              {goLiveChecklist?.summary && (
                <div className={`p-4 rounded-lg border-2 ${
                  goLiveChecklist.summary.is_ready
                    ...
                    {goLiveChecklist.summary.is_ready ? (
                      <>
                        ...
                        <span className={`font-medium ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>
                          All checks passed ({goLiveChecklist.summary.completed}/{goLiveChecklist.summary.total}). Employee is ready for Go-Live!
                        </span>
                      </>
                    ) : (
                      <>
                        ...
                        <span className={`font-medium ${isDark ? 'text-amber-400' : 'text-amber-700'}`}>
                          {goLiveChecklist.summary.completed}/{goLiveChecklist.summary.total} checks complete ({goLiveChecklist.summary.percentage}%)
                        </span>
                      </>
                    )}
                  </div>
              ...
              <DialogFooter className="gap-2">
                ...
                <Button 
                  ...
                  disabled={actionLoading || !goLiveChecklist?.summary?.is_ready}
                  className="bg-emerald-600 hover:bg-emerald-700"
                >
